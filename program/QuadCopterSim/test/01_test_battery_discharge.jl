...
Δt = 0.01;                      #simulation timestep (s)
tf = 10.0 * 60;                 #simulation end time (s)
...
motors_pwm = ones(4,length(t));    #duty cycle of each motor over time
println("Cb(0) = ", state[1,1]/3.6, " mAh");
time0 = time();
for i in 2:lastindex(t)
    ...
    if i%6000 == 0
        println("Cb(",t[i],") = ", state[1,i]/3.6, " mAh");
    end
end
println("Completed in ", round(time()-time0,digits=2), " s");

#plot battery discharge profile
plt1 = plot(t, state[1,:]/3.6,
    title = "Battery discharge profile",
    legend = false,
    xlabel = "Time (s)",
